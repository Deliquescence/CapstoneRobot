...
class Camera_Control(object):
	...
	def __init__(self, debug=_DEBUG, bus_number = 1, pan_channel = CAMERA_PAN_CHANNEL, tilt_channel = CAMERA_TILT_CHANNEL):
		...
		self.debug = debug
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Camera Pan PWM channel: {self._pan_channel}')
			print(f'{self._DEBUG_INFO} Camera Tilt PWN channel: {self._tilt_channel}')
			print(f'{self._DEBUG_INFO} Camera Pan offset value: {self.turning_offset}')
			
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} \n\tLeft pan angle: {self._pan_angle["left"]}\n \tStraight pan angle: {self._pan_angle["straight"]}}\n \tRight pan angle: {self._pan_angle["right"]}')
			print(f'{self._DEBUG_INFO} \n\tDown tilt angle: {self._tilt_angle["down"]}\n \tFlat tilt angle: {self._tilt_angle["flat"]}\n \tUp tilt angle: {.self._tilt_angle["up"]}')
				
	def pan_left(self):
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Execute left pan.')
		self.pan.write(self._pan_angle["left"])

	def pan_straight(self):
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Execute straight pan.')
		self.pan.write(self._pan_angle["straight"])

	def pan_right(self):
		
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Execute right pan.')
		self.pan.write(self._pan_angle["right"])

	def pan_to(self, angle):
		#Pan camera to given horizontal angle
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Pan to {angle}.')
...
def test():
	...
	try:
		while True:
			...
			test_pan_angle = (int(input("Enter a pan value: "))
			test_tilt_angle = (int(input("Enter a tilt value: "))
