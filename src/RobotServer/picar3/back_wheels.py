...
* Update      : Cody Trombley	2019-01-23	  Python 3 port
				Cavon    		2016-09-13    New release
*               Cavon    		2016-11-04    fix for submodules
**********************************************************************
'''

from SunFounder_TB6612_3 import TB6612
from SunFounder_PCA9685_3 import PCA9685
import filedb
...
class Back_Wheels(object):
	...
	def __init__(self, debug = False, bus_number = 1, db = "config"):
		...
		self.debug = debug
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Set left wheel to #{self.Motor_A}, PWM channel to {self.PWM_A}')
			print(f'{self._DEBUG_INFO} Set right wheel to #{self.Motor_B}, PWM channel to {self.PWM_B}')
	...
	def forward(self):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turning both wheels forward.')
	...
	def backward(self):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turning both wheels backward.')
	...
	def stop(self):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Stop both wheels.')
	...
	def speed(self, speed):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Set speed to {self._speed}')
