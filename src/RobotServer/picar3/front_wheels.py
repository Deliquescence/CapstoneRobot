...
class Front_Wheels(object):
	...
	def __init__(self, db = "config", debug = False, bus_number = 1, channel = FRONT_WHEEL_CHANNEL):
		...
		self.debug = debug
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Front wheel PWM channel: {self._channel}')
			print(f'{self._DEBUG_INFO} Front wheel offset value: {self.turning_offset}')

		self._angle = {"left":self._min_angle, "straight":self._straight_angle, "right":self._max_angle}
		if self._DEBUG:
			print(f.'{self._DEBUG_INFO} \nLeft angle: {self._angle["left"]} \nStraight angle: {self._angle["straight"]} \nRight angle: {self._angle["right"]}')

	def turn_left(self):
		''' Turn the front wheels left '''
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn left')
		self.wheel.write(self._angle["left"])

	def turn_straight(self):
		''' Turn the front wheels back straight '''
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn straight')
		self.wheel.write(self._angle["straight"])

	def turn_right(self):
		''' Turn the front wheels right '''
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn right')
		self.wheel.write(self._angle["right"])

	def turn(self, angle):
		''' Turn the front wheels to the giving angle '''
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn to {angle}')
	...
	def debug(self, debug):
		...
		if self._DEBUG:
			...
			print(f'{self._DEBUG_INFO} Set wheel debug on.')
			self.wheel.debug = True
		else:
			print(f'{self._DEBUG_INFO} Set debug off.')
			print(f'{self._DEBUG_INFO} Set wheel debug off.')
			self.wheel.debug = False

	def ready(self):
		''' Get the front wheels to the ready position. '''
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn to "Ready" position.')
	...
	def calibration(self):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn to "Calibration" position.')
		self.turn_straight()
