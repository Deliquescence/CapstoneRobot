...
class Servo(object):
	...
	def _angle_to_analog(self, angle):
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Angle {angle} equals Analog_value {analog_value}')
		return analog_value
	...
	def offset(self, value):
		...
		self._offset = value
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Set offset to {self.offset}')

	def write(self, angle):
		''' Turn the servo within given angle. '''
		if self.lock:
			if angle > 180:
				angle = 180
			if angle < 0:
		...
		if self._DEBUG:
			print(f'{self._DEBUG_INFO} Turn angle = {angle}')
