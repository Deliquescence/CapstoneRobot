...
class PWM(object):
            ...
            for line in f:
                if line.startswith('Revision'):
                    if line[11:-1] in self.RPI_REVISION_0:
                        ...
                        print(f'Error. Pi revision didn\'t recognize, module number: {line[11:-1]}')
                        print(f'Exiting...')
                        quit()
        except Exception, e:
            f.close()
    ...
    def setup(self):
        ...
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Reseting PCA9685 MODE1 (without SLEEP) and MODE2')
    ...
    def _write_byte_data(self, reg, value):
        ...
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Writing value {value} to {reg}')
        try:
    ...
    def _read_byte_data(self, reg):
        ...
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Reading value from {reg}')
        try:
    ...
    def _check_i2c(self):
        ...
        print(f'\nYour Pi Rivision is: {self._get_pi_revision()}')
        print(f'I2C bus number is: {bus_number}')
        print(f'Checking I2C device:')
        cmd = "ls /dev/i2c-%d" % bus_number
        output = commands.getoutput(cmd)
        print(f'Commands "{cmd}" output:')
        print(output)
        if '/dev/i2c-%d' % bus_number in output.split(' '):
            print('I2C device setup OK')
        else:
            print(f'Seems like I2C have not been set, Use 'sudo raspi-config' to set I2C')
        cmd = "i2cdetect -y %s" % self.bus_number
        output = commands.getoutput(cmd)
        print(f'Your PCA9685 address is set to 0x{self.address}')
    ...
    def frequency(self, freq):
        '''Set PWM frequency'''
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Set frequency to {freq}')
        ...
        prescale_value -= 1.0
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Setting PWM frequency to {freq} Hz')
            print(f'{self._DEBUG_INFO} Estimated pre-scale: prescale_value')
        prescale = math.floor(prescale_value + 0.5)
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Final pre-scale: {prescale}')
    ...
    def write(self, channel, on, off):
        ...
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Set channel "{channel}" to value "{off}"')
    ...
    def write_all_value(self, on, off):
        ...
        if self._DEBUG:
            print(f'{self._DEBUG_INFO} Set all channel to value "{off}"')
